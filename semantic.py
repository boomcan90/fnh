...
sentence_pairs = [
    ...
    ["John is very nice.", "John isn't very nice", 0.977],
    ...
    ['Donald Trump just issued a direct threat to the free and independent media', 'Media independence threatened by Trump Administration', 0.75]
]
...
def sentimentAnalysis(str1, str2):

    m = gl.sentiment_analysis.create([str1, str2])
    if (abs(m.predict(data)[0]) >= 0.75 and abs(m.predict(data)[1]) >= 0.75):
        return True
    else:
        return False